...
void CCommandManager::CommandHide(LPOBJ lpObj, char* arg) // OK
{
	...
	{
		gEffectManager->AddEffect(lpTarget,0,EFFECT_INVISIBILITY,0,0,0,0,0);

		gObjViewportListProtocolDestroy(lpTarget);

		gLog->Output(LOG_COMMAND,"[CommandHide][%s][%s] - (State: %d)", lpTarget->Account, lpTarget->Name,1);
	}
	else
	{
		gEffectManager->DelEffect(lpTarget,EFFECT_INVISIBILITY);

		gObjViewportListProtocolCreate(lpTarget);

		gLog->Output(LOG_COMMAND,"[CommandHide][%s][%s] - (State: %d)", lpTarget->Account, lpTarget->Name,0);
	}
