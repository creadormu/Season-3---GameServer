...
void InitLuaFunction(lua_State* L) // OK
{
	...
	lua_register(L,"GetObjectAddLife",LuaGetObjectAddLife);
	lua_register(L,"GetObjectMaxLife",LuaGetObjectMaxLife);
	lua_register(L,"SetObjectLife",LuaSetObjectLife);
	lua_register(L,"SetObjectAddLife",LuaSetObjectAddLife);
...
int LuaGetObjectAddLife(lua_State* L)
{
	if (lua_gettop(L) != 1)
	{
		return luaL_error(L, LUA_SCRIPT_CODE_ERROR1, 1);
	...
	lua_pushinteger(L, (int)gObj[aIndex].AddLife);
	return 1;
};
...
int LuaSetObjectLife(lua_State* L) // OK
{
	...
	gObj[aIndex].Life = aValue;

	return 1;
}

int LuaSetObjectAddLife(lua_State* L) // OK
{
	if (lua_gettop(L) != 2)
	{
		return luaL_error(L, LUA_SCRIPT_CODE_ERROR1, 2);
