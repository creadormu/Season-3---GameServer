...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
	...
	if(lpObj->Class == CLASS_DW)
	{
		if(gServerInfo->m_CalcLuaScriptSwitch != 0)
		{
			...
			gServerInfo->m_CalcCharacter.Execute("DWMagicDamageCalculate","i>iiii",aIndex,&lpObj->MagicDamageMin,&lpObj->MagicDamageMax,&lpObj->CurseDamageMin,&lpObj->CurseDamageMax);
		}
		else
		{
	...
	else if(lpObj->Class == CLASS_DK)
	{
		if(gServerInfo->m_CalcLuaScriptSwitch != 0)
		{
			...
			gServerInfo->m_CalcCharacter.Execute("DKMagicDamageCalculate","i>iiii",aIndex,&lpObj->MagicDamageMin,&lpObj->MagicDamageMax,&lpObj->CurseDamageMin,&lpObj->CurseDamageMax);
		}
		else
		{
	...
	else if(lpObj->Class == CLASS_FE)
	{
		...
		if(gServerInfo->m_CalcLuaScriptSwitch != 0)
		{
			gServerInfo->m_CalcCharacter.Execute("FEMagicDamageCalculate","i>iiii",aIndex,&lpObj->MagicDamageMin,&lpObj->MagicDamageMax,&lpObj->CurseDamageMin,&lpObj->CurseDamageMax);
		}
		else
		{
	...
	else if(lpObj->Class == CLASS_MG)
	{
		if(gServerInfo->m_CalcLuaScriptSwitch != 0)
		{
			...
			gServerInfo->m_CalcCharacter.Execute("MGMagicDamageCalculate","i>iiii",aIndex,&lpObj->MagicDamageMin,&lpObj->MagicDamageMax,&lpObj->CurseDamageMin,&lpObj->CurseDamageMax);
		}
		else
		{
	...
	else if(lpObj->Class == CLASS_DL)
	{
		if(gServerInfo->m_CalcLuaScriptSwitch != 0)
		{
			gServerInfo->m_CalcCharacter.Execute("DLPhysiDamageCalculate","i>iiii",aIndex,&lpObj->PhysiDamageMinRight,&lpObj->PhysiDamageMinLeft,&lpObj->PhysiDamageMaxRight,&lpObj->PhysiDamageMaxLeft);
			gServerInfo->m_CalcCharacter.Execute("DLMagicDamageCalculate","i>iiii",aIndex,&lpObj->MagicDamageMin,&lpObj->MagicDamageMax,&lpObj->CurseDamageMin,&lpObj->CurseDamageMax);
		}
		else
		{
	...
	else if(lpObj->Class == CLASS_SU)
	{
		if(gServerInfo->m_CalcLuaScriptSwitch != 0)
		{
			gServerInfo->m_CalcCharacter.Execute("SUPhysiDamageCalculate","i>iiii",aIndex,&lpObj->PhysiDamageMinRight,&lpObj->PhysiDamageMinLeft,&lpObj->PhysiDamageMaxRight,&lpObj->PhysiDamageMaxLeft);
			gServerInfo->m_CalcCharacter.Execute("SUMagicDamageCalculate","i>iiii",aIndex,&lpObj->MagicDamageMin,&lpObj->MagicDamageMax,&lpObj->CurseDamageMin,&lpObj->CurseDamageMax);
		}
		else
		{
	...
	else if(lpObj->Class == CLASS_RF)
	{
		if(gServerInfo->m_CalcLuaScriptSwitch != 0)
		{
			gServerInfo->m_CalcCharacter.Execute("RFPhysiDamageCalculate","i>iiii",aIndex,&lpObj->PhysiDamageMinRight,&lpObj->PhysiDamageMinLeft,&lpObj->PhysiDamageMaxRight,&lpObj->PhysiDamageMaxLeft);
			gServerInfo->m_CalcCharacter.Execute("RFMagicDamageCalculate","i>iiii",aIndex,&lpObj->MagicDamageMin,&lpObj->MagicDamageMax,&lpObj->CurseDamageMin,&lpObj->CurseDamageMax);
		}
		else
		{
	...
	if(gServerInfo->m_CalcLuaScriptSwitch != 0)
	{
		gServerInfo->m_CalcCharacter.Execute("CalcAttackSuccessRatePvP","i>i",aIndex,&lpObj->AttackSuccessRatePvP);
	}
	else
	{
		if(lpObj->Class == CLASS_DW)
	...
	if(gServerInfo->m_CalcLuaScriptSwitch != 0)
	{
		gServerInfo->m_CalcCharacter.Execute("CalcDefenseSuccessRatePvP","i>i",aIndex,&lpObj->DefenseSuccessRatePvP);
	}
	else
	{
		if(lpObj->Class == CLASS_DW)
	...
	if(lpObj->Class == CLASS_DK || lpObj->Class == CLASS_MG || lpObj->Class == CLASS_DL || lpObj->Class == CLASS_RF)
	{
		if(Right->IsItem() != 0 && Left->IsItem() != 0)
		{
			if(Right->m_Index >= GET_ITEM(0,0) && Right->m_Index < GET_ITEM(4,0) && Left->m_Index >= GET_ITEM(0,0) && Left->m_Index < GET_ITEM(4,0))
			{
				if(gServerInfo->m_CalcLuaScriptSwitch != 0)
				{
					gServerInfo->m_CalcCharacter.Execute("PhysiDamageSpecial","iiiii>iiii",aIndex,lpObj->PhysiDamageMinRight,lpObj->PhysiDamageMaxRight,lpObj->PhysiDamageMinLeft,lpObj->PhysiDamageMaxLeft,&lpObj->PhysiDamageMinRight,&lpObj->PhysiDamageMaxRight,&lpObj->PhysiDamageMinLeft,&lpObj->PhysiDamageMaxLeft);
				}
				else
				{
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv,int aIndex) // OK
{
	...
	for(int n=0;n < MAX_EFFECT_LIST;n++)
	{
		...
		gEffectManager->AddEffect(lpObj,1,effect.m_index,effect.m_count,effect.m_value[0],effect.m_value[1],effect.m_value[2],effect.m_value[3],0);
	}
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,int damage,int DamageType,int flag,int type,int skill,int ShieldDamage,bool combo) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if(lpTarget->Type == OBJECT_MONSTER)
		{
			...
			gScriptLoader->OnMonsterDie(lpTarget->Index,SummonIndex);
		}
